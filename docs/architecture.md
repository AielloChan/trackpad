...
- Protocol models, including pointer, scroll, magnify, tap, and semantic system action events.
- Session state.
...
Configuration sync uses full `TrackpadConfiguration` snapshots as a last-write-wins register for the MVP. Both endpoints keep a local copy, and the macOS host sends its current snapshot when a paired client connects. Applying a remote snapshot updates local consumers without echoing the same value back, which prevents sync loops. The macOS host app is the visible editing surface for tuning. Pointer speed, pointer acceleration, and gesture thresholds are consumed by iOS, while scroll momentum is consumed by macOS. Pointer acceleration is computed from a recent movement window, not from the elapsed time since touch down, so the cursor can return to the slow multiplier during one continuous finger contact. The host app applies tuning changes live but persists them only when the user clicks `Save Configuration`, writing JSON to `~/Library/Application Support/Trackpad/host_configuration.json`.

The iOS client sends only finger-driven scroll input on the hot path. The macOS host can generate two-finger scroll momentum locally after a finger-driven scroll ends, using an axis-locked release velocity derived from the actual sample span inside a tunable final sample window. Slow release velocity is ramped down before applying the user-tuned momentum amount, so slow finger movement does not launch fast inertia. Momentum enablement, amount, decay rate, and tail velocity window are synchronized through the configuration channel; iOS does not enqueue synthetic momentum events over the network. The host integrates an exponential velocity decay against real elapsed time one frame at a time, instead of precomputing a long command queue, so scheduler delays do not replay stale backlog. The macOS scroll injector also carries subpixel residuals into CoreGraphics integer wheel fields so very small tail deltas are not lost. The tuning range is deliberately broad enough for real-device matching against native Apple trackpad inertial scroll distance, and the host app can fully disable synthetic momentum while preserving the tuning values. When any finger touches the iOS surface again, the client sends a `contact.began` boundary event so the host can cancel scheduled momentum immediately before visible pointer or scroll movement occurs.
